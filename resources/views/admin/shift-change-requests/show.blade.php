...
@section('page-title', 'تفاصيل طلب تغيير الوردية')
...
<div class="max-w-2xl space-y-6">
    ...
    <div class="bg-white rounded-2xl shadow-sm p-6">
        ...
        <dl class="space-y-3 text-sm">
            <div class="flex justify-between">
                ...
                <dd class="font-medium text-gray-800">{{ $shiftChangeRequest->employee->user->name }}</dd>
            </div>
            <div class="flex justify-between">
                <dt class="text-gray-500">القسم:</dt>
                <dd class="text-gray-800">{{ $shiftChangeRequest->employee->department?->name_ar ?? '-' }}</dd>
            </div>
            <div class="flex justify-between">
                <dt class="text-gray-500">الوردية الحالية:</dt>
                <dd class="text-gray-800">{{ $shiftChangeRequest->currentShift?->name ?? '-' }}</dd>
            </div>
            <div class="flex justify-between">
                <dt class="text-gray-500">الوردية المطلوبة:</dt>
                <dd class="text-gray-800">{{ $shiftChangeRequest->requestedShift?->name ?? '-' }}</dd>
            </div>
            <div class="flex justify-between">
                <dt class="text-gray-500">تاريخ التنفيذ:</dt>
                <dd class="text-gray-800">{{ $shiftChangeRequest->effective_date->format('Y/m/d') }}</dd>
            </div>
            <div class="flex justify-between">
                <dt class="text-gray-500">السبب:</dt>
                <dd class="text-gray-800">{{ $shiftChangeRequest->reason }}</dd>
            </div>
            <div class="flex justify-between">
                <dt class="text-gray-500">الحالة:</dt>
                <dd>{{ $shiftChangeRequest->status_label }}</dd>
            </div>
        </dl>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="bg-green-50 border border-green-200 rounded-2xl p-5">
            ...
            <form method="POST" action="{{ route('admin.shift-change-requests.approve', $shiftChangeRequest) }}">
                @csrf
    ...
    @elseif($shiftChangeRequest->manager_notes)
    <div class="bg-white rounded-2xl shadow-sm p-5">
        <p class="text-sm text-gray-500 mb-1">ملاحظات الإدارة:</p>
        <p class="text-gray-800">{{ $shiftChangeRequest->manager_notes }}</p>
    </div>
    @endif

    <a href="{{ route('admin.shift-change-requests.index') }}" class="inline-block text-blue-600 hover:underline text-sm">
