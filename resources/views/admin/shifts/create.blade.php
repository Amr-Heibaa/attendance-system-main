...
@section('page-title', 'إضافة وردية')
        ...
        <div class="space-y-5">
            ...
            <div class="grid grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="time" name="start_time" value="{{ old('start_time') }}" class="w-full border rounded-xl px-4 py-2.5 @error('start_time') border-red-400 @enderror">
                    @error('start_time')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">وقت الانتهاء *</label>
                    <input type="time" name="end_time" value="{{ old('end_time') }}" class="w-full border rounded-xl px-4 py-2.5 @error('end_time') border-red-400 @enderror">
                    @error('end_time')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">دقائق السماح *</label>
                    <input type="number" name="grace_minutes" value="{{ old('grace_minutes', 15) }}" class="w-full border rounded-xl px-4 py-2.5 @error('grace_minutes') border-red-400 @enderror">
