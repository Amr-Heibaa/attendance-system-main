...
<div class="bg-white rounded-2xl shadow-sm p-6">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-6">
        <div class="flex gap-3">
            ...
            <a href="{{ route('admin.employees.import.form') }}" class="bg-green-600 hover:bg-green-700 text-white px-5 py-2.5 rounded-xl text-sm font-medium transition">
                استيراد Excel
            </a>
        </div>

        <a href="{{ route('admin.employees.template.excel') }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-5 py-2.5 rounded-xl text-sm font-medium transition">
            تحميل نموذج Excel
        </a>
    </div>
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50 transition">
                    ...
                    <td class="py-3 px-4 font-medium text-gray-800">{{ $employee->user->name }}</td>
                    <td class="py-3 px-4 text-gray-600">{{ $employee->phone ?? '-' }}</td>
                    <td class="py-3 px-4 text-gray-600">{{ $employee->national_id ?? '-' }}</td>
                    <td class="py-3 px-4 text-gray-600">{{ $employee->department?->name_ar ?? '-' }}</td>
                    <td class="py-3 px-4 text-gray-600">{{ $employee->work_location ?? '-' }}</td>
                    <td class="py-3 px-4">
                        @php
                            $statusClasses = [
