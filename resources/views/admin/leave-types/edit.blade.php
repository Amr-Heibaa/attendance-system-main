...
@section('page-title', 'تعديل نوع إجازة')
        ...
        <div class="space-y-5">
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input name="name" value="{{ old('name', $leaveType->name) }}" class="w-full border rounded-xl px-4 py-2.5 @error('name') border-red-400 @enderror">
                    @error('name')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">الاسم (AR) *</label>
                    <input name="name_ar" value="{{ old('name_ar', $leaveType->name_ar) }}" class="w-full border rounded-xl px-4 py-2.5 @error('name_ar') border-red-400 @enderror">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="max_days_per_year" value="{{ old('max_days_per_year', $leaveType->max_days_per_year) }}"
                ...
                <div class="flex items-end gap-6">
                    <label class="flex items-center gap-2 text-sm text-gray-700">
                        <input type="checkbox" name="is_paid" value="1" {{ old('is_paid', $leaveType->is_paid) ? 'checked' : '' }}>
                        مدفوعة
                    </label>
                    <label class="flex items-center gap-2 text-sm text-gray-700">
