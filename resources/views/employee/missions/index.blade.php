...
<div class="bg-white rounded-2xl shadow-sm p-6">
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-xl font-bold text-gray-800">طلبات المهمات</h2>
        <a href="{{ route('employee.missions.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl text-sm font-medium transition">
            + طلب مهمة جديد
        </a>
    </div>
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50">
                    <td class="py-3 px-4 text-gray-800 font-medium">{{ $req->title }}</td>
                    <td class="py-3 px-4 text-gray-700">{{ $req->type_label }}</td>
                    <td class="py-3 px-4 text-gray-700">{{ $req->date->format('Y/m/d') }}</td>
                    <td class="py-3 px-4 text-gray-700">{{ \Carbon\Carbon::parse($req->from_time)->format('H:i') }} - {{ \Carbon\Carbon::parse($req->to_time)->format('H:i') }}</td>
                    <td class="py-3 px-4 text-gray-700">{{ $req->location ?? '-' }}</td>
                    <td class="py-3 px-4">
                ...
                @empty
                <tr>
                    <td colspan="6" class="text-center py-12 text-gray-400">لا توجد طلبات مهمات</td>
                </tr>
                @endforelse
            </tbody>
        </table>
